...
--   Exercises
	...
	-- 6. Get the Longest Album
     ...
     DELETE FROM albums WHERE id=8;  -- 8th record deleted
         
     UPDATE albums SET release_year =1986 WHERE release_year is null;
     
	-- 8. Insert a record for your favorite Band and one of their Albums
    
    INSERT INTO albums (name,release_year,band_id) VALUES ('Peddi',2026,2);
    
     -- 10. Get the Average Length of all Songs
     select avg(length) as Avf_Song_Duration
     from songs;
...
SELECT b.name, count(s.id) songs_count
from albums a
